...
##N-1 comp amp variables, the first one is == start

###line inits== the initial value and limit for the priors in emcee, and the step size
OIIILineInits = {'amp':[0,5000,0.01],'width':[1,30,0.01],'compAmps':[-10,10,0.0001],'dlam':[-40,40,0.01]}
startOIIIAmp = 1.48

OIII_A = {'amp':startOIIIAmp, 'width':3.05,    'compAmps':[],'dlam':-2.348}

OIII_B = {'amp':0.80, 'width':'fixed','compAmps':'fixed','dlam':'fixed'}
OIII_C = {'amp':0.99, 'width':'fixed','compAmps':'fixed','dlam':'fixed'}
OIII_D = {'amp':1.19, 'width':'fixed','compAmps':'fixed','dlam':'fixed'}

startHBetaAmp = 1.42
HBetaLineInits= {'amp':[0,10000,0.001],'width':[1,600,0.01],'compAmps':[-10,10,0.0001],'dlam':[-40,40,0.01]}

HBeta_A = {'amp':startHBetaAmp, 'width':34.7,'compAmps':[],'dlam':-2.2}
HBeta_B = {'amp':0.82, 'width':34.7,'compAmps':'fixed','dlam':'fixed'}
HBeta_C = {'amp':1.15, 'width':34.7,'compAmps':'fixed','dlam':'fixed'}
HBeta_D = {'amp':0.76, 'width':34.7,'compAmps':'fixed','dlam':'fixed'}
...
#FeA = {'amp': startFeAmp, 'velocity':startFeVelocity,'G':0.88,'IZw1':0.64,'S':2.57,'dlam':25.6}
#FeB = {'amp': 2.4, 'velocity':'fixed','G':'fixed','IZw1':'fixed','S':'fixed','dlam':'fixed'}
#FeC = {'amp': 1.52, 'velocity':'fixed','G':'fixed','IZw1':'fixed','S':'fixed','dlam':'fixed'}
#FeD = {'amp': 1.8, 'velocity':'fixed','G':'fixed','IZw1':'fixed','S':'fixed','dlam':'fixed'}
...
#zero = 5012
continuumInits = {'amp':[0,100,0.001],'slope':[-0.1,0.1,1e-06]}

continuumA = {'amp':startContinuumAmp,'slope':startContinuumSlope}
continuumB = {'amp':0.019,'slope':2.5e-05}
continuumC = {'amp':0.016,'slope':2.6e-05}
continuumD = {'amp':0.012,'slope':2.11e-05}
...
specA = {'HBeta':HBeta_A,'OIII':OIII_A,'continuum':continuumA, 'HBetaNarrow_ind':HBetaNarrow_ind_A}
specB = {'HBeta':HBeta_B,'OIII':OIII_B,'continuum':continuumB, 'HBetaNarrow_ind':HBetaNarrow_ind_B}
specC = {'HBeta':HBeta_C,'OIII':OIII_C,'continuum':continuumC, 'HBetaNarrow_ind':HBetaNarrow_ind_C}
specD = {'HBeta':HBeta_D,'OIII':OIII_D,'continuum':continuumD, 'HBetaNarrow_ind':HBetaNarrow_ind_D}


agnCompsInitDict = {'HBeta':HBetaLineInits,'OIII':OIIILineInits,'continuum':continuumInits, 'HBetaNarrow_ind':HBetaNarrowLineInits}
...
specBase = '/home/anierenberg/repositories/OSIRIS_QUADS' \
           '/mcmcPositions/1138/specs/1138_770_1540_myWeight_median_nPix11_inc2012_all_dsmallsig_2amp_5_770_1540_mean_'

dats = []
sigs = []

### this opens each spectroscopy file and reads the data. Note the order of the arrays it expects to readout,
# adjust this according to however you have the arrays stored (you will be reading in a fits file rather than a pickle file).
...
for specName in agnSpecs.keys():
    lamRest,data,sig = pickle.load(open(specBase+specName+'_inc2012_noise.pickle','rb'))
    dats.append(data)
    #print('data')
    #print(data)
...
#oFiName = 'specPosteriors/1138_ABCD_HB0_dHBW_OIII0.pickle'
oFiName = 'specPosteriors/1138_ABCD_HB0_dHBW_OIII0.pickle'
...
#oFiName = 'specPosteriors/0259_ABCD_HB1_OIII1_HBN_ind.pickle'
...
specs = agnSpectrumClass.getChainComponents(bestFit,agnSpecs)
...
agnSpectrumClass.plotVals(lamRest, bestFit,dats,sigs,agnSpecs)
